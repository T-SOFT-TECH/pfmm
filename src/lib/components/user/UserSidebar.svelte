<script lang="ts">
	...
	const menuItems = [
		...
		{ href: '/user/projects', label: 'My Projects', icon: '📁' },
		{ href: '/user/courses', label: 'My Courses', icon: '📚' },
		{ href: '/user/certificates', label: 'Certificates', icon: '📜' },
		{ href: '/user/messages', label: 'Messages', icon: '✉️' },
...
<aside class="w-64 bg-dark-900 border-r border-dark-800 fixed h-full z-30 transition-transform {sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0">
	...
	<div class="p-6 border-b border-dark-800">
		<div class="flex items-center space-x-3">
			...
			<div>
				<div class="text-accent-400 font-bold text-lg">User Portal</div>
		...
		<ul class="space-y-2">
			{#each menuItems as item}
				<li>
					<a
						href={item.href}
						class="flex items-center space-x-3 px-4 py-3 rounded-lg text-dark-300 hover:bg-dark-800 hover:text-accent-400 transition-colors"
					>
